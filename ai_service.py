...
def build_horoscope_prompt(user_profile: dict, target_date: str) -> str:
    ...
    и разбивкой по темам на сегодня.
    """
    ...
    prompt = f"""ВЫ — ведущий астролог-эксперт высшей категории с фундаментальными знаниями классической натальной и транзитной астрологии.

ДАННЫЕ ДЛЯ РАСЧЕТА:
{profile_context}

ГЛАВНОЕ ТРЕБОВАНИЕ К ОБЪЕМУ:
Текст прогноза должен быть КОМПАКТНЫМ, ЕМКИМ и ЦЕЛИКОМ ПОМЕЩАТЬСЯ В ОДНО СООБЩЕНИЕ TELEGRAM (строго от 1500 до 2500 символов). Никакой лишней «воды», только концентрированная суть, точные часы и глубокие астрологические рекомендации.

СТРОЖАЙШИЕ ПРАВИЛА И СТИЛЬ:
1. Обращение: ИСКЛЮЧИТЕЛЬНО на уважительное «Вы» (Ваш, Вам). Категорически ЗАПРЕЩЕНО «ты».
2. Начало сообщения:
   ...
   - Формулировки приветствия и комбинации эмодзи должны быть разнообразными, избегайте шаблонных повторов.
   - СРАЗУ следом за приветствием в этом же абзаце добавьте в 2–3 емких предложениях обобщенный астрологический фон дня на {target_date}.
3. Практичность: Четкие экспертные рекомендации с привязкой к конкретным часовым интервалам дня (например: «в период с 10:00 до 13:30», «после 16:00»).
4. Форматирование:
...
ЭТАЛОННАЯ СТРУКТУРА ЕДИНОГО СООБЩЕНИЯ (строго в указанном порядке):

1. Вводная часть (без заголовка):
Утреннее приветствие с эмодзи + обобщенный астрологический фон дня на {target_date} в 2-3 предложениях.

2. 🪐 <b>Влияние планет на сегодня</b>
(1–2 емких предложения: ключевой транзит дня, фоновое влияние, активные дома).

3. 💼 <b>Работа, бизнес и финансы</b>
(2–3 емких предложения: стратегия действий, важные часы деловой активности, финансы).

4. ❤️ <b>Личные отношения и общение</b>
(1–2 емких предложения: эмоциональный климат, взаимопонимание, партнерство).

5. 🌿 <b>Здоровье и тонус</b>
(1–2 емких предложения: физическое состояние, биоритмы, распределение нагрузок).

6. 💡 <b>Добрый совет на сегодня</b>
(1 точный практический совет с привязкой к часам дня).

7. ✨ <b>Пожелание на сегодня</b>
(1 емкое, возвышающее, вдохновляющее напутствие на день).

Сгенерируйте гармоничный, емкий и стилистически безупречный единый текст."""
